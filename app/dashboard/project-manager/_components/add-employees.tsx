...
export default function AddEmployees() {
    ...
    function onSubmit(values: z.infer < typeof formSchema > ) {
          setSelected([...selected, {employee:filterData(values.employee_list), role:values.role }])
    }
    ...
    return (
        <section className="grid grid-cols-12 gap-4 space-y-2 max-w-3xl mx-auto py-10">
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormDescription>Select employees you want on the project</FormDescription>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                                />
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormDescription>The assigned role on the project</FormDescription>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                                />
            ...
            <section className="col-span-6 rounded-md bg-slate-800 p-6 flex flex-col justify-between">
                <ul className="flex flex-col gap-4">

                {
                    selected?.map(employee => (
                        <li key={v4()} className="p-2 bg-slate-900 rounded-md flex items-center gap-4 text-sm flex-wrap">
                            <span>Name - {`${employee.employee.firstname} ${employee.employee.lastname}`}</span> /
                ...
                {selected.length > 0 && (
                    <button className="text-sm border px-2 py-1 rounded-md bg-blue-900">
